...
export const signIn = async (req: Request, res: Response) => {
  ...
  let { email, password } = req.body.requestData as {
    email: string;
    password: string;
  };
  ...
  if (user.google_auth) {
    return res
      .status(401)
      .json({ error: "Please log in using your google account" });
  }
...
export const signUp = async (req: Request, res: Response) => {
  ...
  if (fullname.length < 3) {
    return res
      .status(400)
      .json({ error: "Fullname must be at least 3 letters" });
  }
  ...
  if (password !== passwordRepeat) {
    return res
      .status(400)
      .json({ error: "Password and repeat password is different" });
  }
  ...
  try {
    ...
    if (!user) {
      ...
      if (!user.google_auth) {
        res.status(403).json({
          error:
            "This email was signed up without Google. Please use email and password to log in.",
        });
      } else {
        req.session.user = {
          _id: user._id,
